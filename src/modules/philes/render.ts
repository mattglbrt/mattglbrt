...
export type PhileBodyBlock = { kind: "text"; html: string } | { kind: "image"; src: string; alt: string };
...
export function renderPhileBodyBlocks(phile: Phile): PhileBodyBlock[] {
  return splitBodyBlocks(phile.body ?? "").map((block) => {
    if (block.kind === "image") {
      // Optimization happens in PhileShell (Astro's image pipeline); keep the
      // raw src/alt here. renderImage() remains the fallback for images that
      // can't be bundled (remote URLs or paths outside src/images).
      return {
        kind: "image",
        src: block.src,
        alt: block.alt
...
export function renderImage(src: string, alt: string): string {
